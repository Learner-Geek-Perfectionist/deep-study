...
# ---------------------------------------------------------------------------
# Type profiles: source / protocol / language
# ---------------------------------------------------------------------------
...
TYPE_SIGNATURES = {
    "source": "03-core-data-structures.md",
...
def get_required_files(study_type: str) -> list[str]:
    return COMMON_FILES + TYPE_SPECIFIC_FILES[study_type]


def get_required_headings(study_type: str) -> dict[str, list[str]]:
    headings = dict(COMMON_HEADINGS)
    headings.update(TYPE_SPECIFIC_HEADINGS[study_type])
    return headings


def get_entry_rules(study_type: str) -> dict[str, tuple[str, int]]:
    ...
    if study_type is None:
        ...
        if study_type is None:
            return [
                ...
                "03-core-data-structures.md (source), "
                "03-message-formats.md (protocol), "
                "03-representations.md (language)"
            ]

    if study_type not in TYPE_SPECIFIC_FILES:
        return [f"Unknown study type: {study_type}. Must be one of: source, protocol, language"]

    required_files = get_required_files(study_type)
    required_headings = get_required_headings(study_type)
    entry_rules = get_entry_rules(study_type)
