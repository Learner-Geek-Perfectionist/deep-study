...
REQUIRED_HEADINGS = {
    "source": {
        "00-overview.md": ["## \u7814\u7a76\u8303\u56f4\u4e0e\u6838\u5fc3\u95ee\u9898", "## \u5165\u53e3\u4e0e\u6750\u6599\u5730\u56fe", "## \u6838\u5fc3\u6982\u5ff5", "## \u7ec4\u4ef6\u5173\u7cfb\u4e0e\u5206\u5c42", "## \u4e3b\u8981\u6570\u636e\u6d41"],
        "01-core-data-structures.md": ["## \u6570\u636e\u7ed3\u6784\u6e05\u5355", "## \u5b57\u6bb5\u4e0e\u804c\u8d23", "## \u4e0d\u53d8\u91cf", "## \u6240\u6709\u6743\u4e0e\u751f\u547d\u5468\u671f\u5173\u7cfb", "## \u7ed3\u6784\u4e4b\u95f4\u7684\u4f9d\u8d56\u5173\u7cfb"],
        "02-core-functions.md": ["## \u5173\u952e\u51fd\u6570\u6e05\u5355", "## \u51fd\u6570\u804c\u8d23\u4e0e\u8f93\u5165\u8f93\u51fa", "## \u5173\u952e\u72b6\u6001\u53d8\u5316", "## \u8c03\u7528\u5173\u7cfb", "## \u5f02\u5e38\u8def\u5f84\u4e0e\u5931\u8d25\u5904\u7406"],
        "03-mechanisms.md": ["## \u673a\u5236\u5217\u8868", "## \u673a\u5236\u4e00", "## \u673a\u5236\u4e8c", "## \u673a\u5236\u4e4b\u95f4\u5982\u4f55\u534f\u4f5c", "## \u8bbe\u8ba1\u7ea6\u675f\u4e0e Trade-off"],
        "04-control-flow-and-state.md": ["## \u4e3b\u63a7\u5236\u6d41", "## \u5173\u952e\u72b6\u6001\u8f6c\u6362", "## \u5e76\u53d1\u4e0e\u540c\u6b65\u70b9", "## \u8de8\u6a21\u5757\u63a7\u5236\u4ea4\u63a5"],
        "05-end-to-end-trace.md": ["## \u573a\u666f\u5b9a\u4e49", "## \u5165\u53e3\u6761\u4ef6", "## \u6267\u884c\u6b65\u9aa4", "## \u72b6\u6001\u53d8\u5316", "## \u573a\u666f\u7ed3\u8bba"],
        "06-failure-and-edge-cases.md": ["## \u5f02\u5e38\u8def\u5f84\u6e05\u5355", "## \u8fb9\u754c\u6761\u4ef6", "## \u8d44\u6e90\u91ca\u653e\u4e0e\u6e05\u7406", "## \u6613\u9519\u70b9"],
        "07-synthesis.md": ["## \u8bbe\u8ba1\u54f2\u5b66", "## \u5173\u952e Trade-off", "## \u53cd\u590d\u51fa\u73b0\u7684\u6a21\u5f0f", "## \u6838\u5fc3\u7ed3\u8bba"],
    },
    "protocol": {
...
def make_valid_study(root, study_type):
    ...
    for name in all_files:
        ...
        if name == "README.md":
            links = [f"- [{t}](./{t})" for t in all_files if t != "README.md"]
...
class ValidateSourceTests(unittest.TestCase):
    def test_valid_source_study_passes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "source")
            self.assertEqual(module.validate_study(d), [])
    def test_missing_required_file_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "source")
            (d / "02-core-functions.md").unlink()
            self.assertTrue(any("02-core-functions.md" in e for e in module.validate_study(d)))
    def test_missing_heading_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "source")
            t = d / "01-core-data-structures.md"
            t.write_text(t.read_text(encoding="utf-8").replace("## \u4e0d\u53d8\u91cf", "## \u5b57\u6bb5\u5217\u8868"), encoding="utf-8")
            self.assertTrue(any("## \u4e0d\u53d8\u91cf" in e for e in module.validate_study(d)))
    def test_missing_entry_count_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "source")
            write_markdown(d / "02-core-functions.md", REQUIRED_HEADINGS["source"]["02-core-functions.md"], mermaid=True, entry_prefix="### \u51fd\u6570\uff1a", entry_count=3)
            self.assertTrue(any("\u81f3\u5c11\u5305\u542b 8 \u4e2a" in e for e in module.validate_study(d)))
    def test_missing_mermaid_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "source")
            write_markdown(d / "00-overview.md", REQUIRED_HEADINGS["source"]["00-overview.md"], mermaid=False)
            self.assertTrue(any("00-overview.md" in e and "Mermaid" in e for e in module.validate_study(d)))
    def test_readme_missing_link_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "source")
            (d / "README.md").write_text("# README\n", encoding="utf-8")
            self.assertTrue(any("README.md" in e for e in module.validate_study(d)))

class ValidateProtocolTests(unittest.TestCase):
    def test_valid_protocol_study_passes(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "protocol")
            self.assertEqual(module.validate_study(d), [])
    def test_missing_message_formats_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "protocol")
            (d / "03-message-formats.md").unlink()
            self.assertTrue(any("03-message-formats.md" in e for e in module.validate_study(d, study_type="protocol")))
    def test_missing_protocol_heading_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "protocol")
            t = d / "04-state-machines.md"
            t.write_text(t.read_text(encoding="utf-8").replace("## \u72b6\u6001\u6e05\u5355", "## \u5176\u4ed6\u6807\u9898"), encoding="utf-8")
            self.assertTrue(any("## \u72b6\u6001\u6e05\u5355" in e for e in module.validate_study(d)))
    def test_missing_format_entry_count_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "protocol")
            write_markdown(d / "03-message-formats.md", REQUIRED_HEADINGS["protocol"]["03-message-formats.md"], mermaid=True, entry_prefix="### \u683c\u5f0f\uff1a", entry_count=2)
            self.assertTrue(any("\u81f3\u5c11\u5305\u542b 5 \u4e2a" in e for e in module.validate_study(d)))

class ValidateLanguageTests(unittest.TestCase):
    def test_valid_language_study_passes(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "language")
            self.assertEqual(module.validate_study(d), [])
    def test_missing_representations_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "language")
            (d / "03-representations.md").unlink()
            self.assertTrue(any("03-representations.md" in e for e in module.validate_study(d, study_type="language")))
    def test_missing_language_heading_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "language")
            t = d / "04-pipeline-stages.md"
            t.write_text(t.read_text(encoding="utf-8").replace("## \u7ba1\u7ebf\u9636\u6bb5\u6e05\u5355", "## \u5176\u4ed6\u6807\u9898"), encoding="utf-8")
            self.assertTrue(any("## \u7ba1\u7ebf\u9636\u6bb5\u6e05\u5355" in e for e in module.validate_study(d)))
    def test_missing_stage_entry_count_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "language")
            write_markdown(d / "04-pipeline-stages.md", REQUIRED_HEADINGS["language"]["04-pipeline-stages.md"], mermaid=True, entry_prefix="### \u9636\u6bb5\uff1a", entry_count=1)
            self.assertTrue(any("\u81f3\u5c11\u5305\u542b 4 \u4e2a" in e for e in module.validate_study(d)))
...
class AutoDetectTests(unittest.TestCase):
    def test_auto_detect_source(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "source")
            self.assertEqual(module.detect_study_type(d), "source")
    def test_auto_detect_protocol(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "protocol")
            self.assertEqual(module.detect_study_type(d), "protocol")
    def test_auto_detect_language(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); make_valid_study(d, "language")
            self.assertEqual(module.detect_study_type(d), "language")
    def test_undetectable_type_fails(self):
        module = load_validator_module()
        with tempfile.TemporaryDirectory() as tmpdir:
            d = Path(tmpdir); (d / "random.md").write_text("# Test", encoding="utf-8")
            self.assertTrue(any("Cannot detect study type" in e for e in module.validate_study(d)))
